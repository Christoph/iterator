...
2. **`iterator-consolidate` could advance `last_memorized_commit`** — **fixed**
...
5. **`/iterator-memorize` range included memory-only bookkeeping commits** — **fixed**
...
- `5fdde93` — `feature(okf-gather-staleness-range): Honor glob memory anchors and ignore memory-only memorize ranges`
- `32200ac` — `feature(okf-writer-invariants): Enforce OKF apply-review invariants`
- `ab47471` — `feature(commit-memory-reviewability): Make commit memory proposals human-reviewable`
- `c64f249` — `feature(commit-memory-reviewability): document reviewable memory updates`
- Bookkeeping commits record feature status and commit shas in `memory/`.

Current iterator plan state after the fixes: **11/11 features done**.
...
- If merging back to `main`, review the feature commits plus bookkeeping commits together.
