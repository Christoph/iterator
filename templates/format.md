...
| `size` / `lines_estimate` | yes | Estimate the *expected diff* (changed lines of **logic + tests** — a chunk contains its own tests; comment/doc-only lines don't count) from `files` + the implementation notes, not gut feel. Target ~100–300 lines: below ~50 the flow overhead outweighs the chunk (merge it), above ~300 it cannot be reviewed (split it) — the writer warns outside that window. `large` chunks get a ⚠️ in the UIs. |
