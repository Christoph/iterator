...
Use this template to generate the self-contained HTML review page for `/review`.
...
<head>
  ...
  <title>local-review — [branch]</title>
  <style>
    /* === Design tokens === */
    [data-theme="dark"] {
      --bg: #0d1117;
...
function renderSidebar() {
  ...
  if (!hasPlanFile) {
    ...
    banner.textContent = 'No PLAN.md features found. Run /plan-features first.';
...
function selectFeature(name) {
  ...
  if (isUncategorized) {
    feature = {
      name: '__uncategorized__',
      description: 'Files not matched by any feature in PLAN.md. Run /plan-features to assign them.',
      blastRadius: null,
      dependsOn: [],
  ...
  detail.innerHTML = `
    ...
    ${showWarning ? `<div class="size-warning">⚠️ This feature has ${totalChanged} changed lines — larger than the 200-line review guideline. Consider splitting it in /plan-features.</div>` : ''}
