...
function start(data) {
  ...
  server.listen(port, '127.0.0.1', () => {
    ...
    exec(`${opener} "${url}"`);
    process.stderr.write(`iterator: listening on ${url}\n`);
  });

  // 2-hour timeout — exits cleanly if no submission
  setTimeout(() => {
    process.stderr.write('iterator: timeout (2h), no feedback received\n');
...
<head>
  ...
  <title>iterator — ${data.branch || 'HEAD'}</title>
  <style>
    [data-theme="dark"] {
...
<div class="main">
  ...
  <div class="detail" id="detail">
    <div class="empty"><h3>Select a feature to review</h3></div>
  </div>
</div>
...
const D = ${json};
// features is the current key; fall back to the legacy features key
...
renderSidebar();
const first = (D.features||[])[0] || (D.uncategorized?.length ? {name:'__unc__'} : null);
...
function renderSidebar() {
  ...
  if (!D.hasFeaturesFile) {
    ...
    b.textContent = 'No features — run /iterator-plan-features first';
    sb.appendChild(b);
  }
...
function selectFeature(name) {
  ...
  document.getElementById('detail').innerHTML = \`
    <div class="fh">
      ...
      <button class="note-btn" onclick="toggleNote('\${name}')">\${note?'Edit note':'+ Add feature note'}</button>
      <div class="note-area \${note?'open':''}" id="na-\${name}">
        <textarea id="nta-\${name}" placeholder="Note about this feature…">\${esc(note)}</textarea>
        <div class="na">
    ...
    \${total>200?'<div class="warn">⚠️ '+total+' changed lines — exceeds 200-line guideline. Consider Split in /iterator-plan-features.</div>':''}
    <div class="meta">
...
function buildFeedbackObj() {
  const features = [];
  const allNames = [...(D.features||[]).map(f=>f.name), (D.uncategorized?.length?'__unc__':null)].filter(Boolean);
...
function updateFb() {
  ...
  if (!hasAny) {
    fbo.innerHTML='<span class="fbe">Add comments or mark features to generate feedback…</span>';
