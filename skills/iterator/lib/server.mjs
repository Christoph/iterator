		...
		if (req.method === "GET" && url.pathname === "/") {
			...
			if (r && r !== RUN_ID) {
				process.stderr.write(
					"iterator: ignored /submit from a previous run’s tab\n",
				);
				res.writeHead(409);
				res.end();
				return;
			...
			req.on("end", async () => {
				...
				if (!done) {
					...
					try {
						...
						parsed = {
							type: "error",
							error: "malformed /submit body (not JSON)",
						};
					}
			...
			req.on("end", () => {
				...
				if (r && r !== RUN_ID) {
					process.stderr.write(
						"iterator: ignored /cancel from a previous run’s tab\n",
					);
					return;
				}
...
export function doneHtml(msg = "Sent to Agent") {
