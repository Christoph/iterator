 ...
 * (input/output/cache read/cache write) plus per-chunk rollups and the plan
 ...
 *                       chunks: { <slug>: {input,output,cacheRead,cacheWrite,turns} } },
...
if(!D.exists || !steps.length){
  ...
  const chunks = Object.entries(T.chunks||{});
  if(chunks.length){
    html += '<div class="sec-title">Per chunk</div>'+table(chunks, 'chunk');
  }
  m.innerHTML = html;
}
