...
.fast-track{display:inline-flex;margin:0 0 var(--sp-3);padding:3px 9px;border-radius:12px;background:var(--bg-green);color:var(--dot-green);font:600 var(--fs-xs) var(--font-mono)}
body.fast-plan .cmt-btn,body.fast-plan .comment-section{display:none}
...
if(D.fastTrack){
  document.body.classList.add('fast-plan');
  document.getElementById('fast-track').hidden = false;
  document.getElementById('plan-hint').innerHTML = 'This complete plan skipped discovery and is ready for review. <strong>Click any section to edit it directly</strong>, confirm the external dependencies, then Accept to save it and continue to feature breakdown.';
}
