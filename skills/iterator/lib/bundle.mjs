 ...
 * (skills/okf-init/write.mjs), and validateBundle.
 */
import {
...
/**
 * Concepts whose `files:` anchors intersect a chunk's `files` globs (or a
 * plain path list). Both sides may be globs, so the match is bidirectional:
 * an anchor `lib/server.mjs` matches a chunk glob `lib/*.mjs`, and an anchor
 * `lib/views/*` matches a chunk path `lib/views/hub.mjs`.
 */
...
// ---------------------------------------------------------------------------
// Knowledge areas (shared with the okf skills — one copy, not three)
