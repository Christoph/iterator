...
export const nowIso = () =>
	process.env.ITERATOR_NOW || new Date().toISOString();
...
export function backlogIndex(items = []) {
	...
	const summary = normalized.length
		? normalized
				.map(
					(item) =>
						`* [${item.kind}] ${item.title}${item.selected ? " — selected" : ""}`,
				)
				.join("\n")
		: "(empty)";
...
export function frontmatter(text, { strict = false } = {}) {
	...
	for (const line of m[1].split(/\r?\n/)) {
		...
		if (kv) {
			...
			const val = kv[2].trim();
			if (val === "")
				fm[key] = null; // may be followed by a block list
 ...
 */
export function updateRootIndex(
	memDir,
	touchedAreas = [],
	{ advanceTo = null } = {},
) {
...
export function mergeRootIndex(existing, links) {
	...
	for (const [target, line] of links) {
		...
		if (i !== -1) {
			lines[i] = line;
			lastBullet = Math.max(lastBullet, i);
