...
function acceptCommit(payload, root) {
	...
	for (const e of entries) {
		...
		gitW(
			["add", "-A", "--", ...paths, ...(memStageable ? [b.memName] : [])],
			b.root,
		);
...
export function applyOp(payload, root) {
	...
	if (!ops[op])
		fail(
			`unknown op '${payload.op || payload.type || ""}' (plan|chunks|design|update-chunk|adjustments|memorize|apply-review|refresh-format|retire-plan|accept-commit|record-review)`,
		);
