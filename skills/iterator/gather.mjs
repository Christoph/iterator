...
export function gatherImplement(startDir) {
	...
	const pending = b.chunks.filter(
		(c) => (c.fm.status || "pending") === "pending",
	);
	const drafts = b.chunks
...
export function gatherRange(startDir) {
	...
	const commits =
		effectiveBase && head
			? git(
					["log", "--format=%H%x09%s", `${effectiveBase}..HEAD`, ...pathspec],
					b.root,
				)
					.split("\n")
					.filter(Boolean)
...
function resolveChunkCommits(root, c) {
	...
	return git(
		["log", "--reverse", "--format=%H", "--grep", `^Chunk: ${c.slug}$`],
		root,
	)
		.split("\n")
		.filter(Boolean);
...
export function gatherReview(startDir, opts = {}) {
	...
	let diffText = hasHead
		? git(["diff", "HEAD"], b.root)
		: git(["diff"], b.root);
	let commitLabel = git(["log", "-1", "--format=%h %s"], b.root);
	let source = "working-tree";

	const selected = opts.chunk
		? b.chunks.filter((c) => c.slug === opts.chunk)
		: b.chunks;
	...
	for (const f of parsed) {
		const owner =
			owners.find((o) => o.tests.has(f.path)) ||
...
if (
	...
	if (!steps[step]) {
		process.stderr.write(
			`iterator gather: unknown step '${step}' (hub|plan|chunk|implement|memorize|range|knowledge|test|review)\n`,
		);
		process.exit(1);
	}
