#!/usr/bin/env node
/**
 * local-review: implementer server
 ...
 *
 * Input:  { branch, chunk: {name, description, implementationNotes}, diff: [{path,hunks}], summary }
...
function start(data) {
  ...
  server.listen(port, '127.0.0.1', () => {
    ...
    exec(`${opener} "${url}"`);
    process.stderr.write(`local-review: implementer listening on ${url}\n`);
  });

  setTimeout(() => {
    process.stderr.write('local-review: timeout (2h), no submission\n');
...
<head>
  ...
  <title>local-review — implement — ${data.branch || 'HEAD'}</title>
  <style>
...
document.getElementById('title').textContent = chunk.name || 'Implemented chunk';
document.getElementById('desc').textContent = chunk.description || '';
if (chunk.implementationNotes)
  document.getElementById('notes-wrap').innerHTML = '<div class="lbl">Implementation notes</div><div class="notes">'+esc(chunk.implementationNotes)+'</div>';
if (D.summary)
...
async function submitReview(){
  ...
  const payload = comment
    ? { type:'review-feedback', chunk: chunk.name, comment }
    : { type:'accept-commit', chunk: chunk.name };
  try {
