...
**pi mode:** if the tools `iterator_gather` / `iterator_write` are available,
use them instead of the shell pipelines below:
`iterator_gather { step: "implement" }` replaces the gather pipe and
`iterator_write` the write.mjs heredoc. Payloads and rules are unchanged.
...
- `/iterator-plan` creates the `memory/` bundle this skill writes into.
- `/iterator-implement` invokes this skill's rules automatically for chunks
  with UI surface; the gather payload's `designFile` tells it whether params
  exist.
- `/iterator-review` reviews the resulting diff like any other chunk work.
