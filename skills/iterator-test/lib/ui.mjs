...
const SHARED_JS = `
// Per-run auth token, baked into the opened URL by lib/server.mjs; every
// request must echo it or the server responds 403 (see "Security" there).
const __TOKEN = new URLSearchParams(location.search).get('t') || '';
function __api(path){ return path + (path.indexOf('?')>=0 ? '&' : '?') + 't=' + encodeURIComponent(__TOKEN); }
let __submitted = false;
function sendCancel(){ if(__submitted) return; __submitted = true;
  try{ navigator.sendBeacon(__api('/cancel'),'{}'); }catch(e){} }
window.addEventListener('pagehide', sendCancel);
async function cancelFlow(){ __submitted = true;
  try{ await fetch(__api('/cancel?now=1'),{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'}); }catch(e){}
...
async function post(payload, okMsg){
  ...
  try{
    await fetch(__api('/submit'),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
