...
└── chunks/
    ...
    ├── archive/      # retired plans: <created>-<slug>/ with plan.md + chunks + usage.md
...
| `memories` | no | Writer-computed at chunk time: the knowledge concept ids (`<area>/<slug>`) whose `files:` anchors match this chunk's `files` — the implementer's reading list. Never hand-authored. |
| `conflicts` | no | JSON scalar `[{"decision","note"}]` — decision concepts this chunk contradicts, flagged by the chunking model; rendered as a red badge and mirrored readably in the `# Decision conflicts` body section. Escalates instead of implementing in auto mode. |

Body sections `# Implementation notes`, `# Snippets`, `# Depends on`,
`# Blast radius` are written at chunk-creation time; `# Decision conflicts`
...
memories. The `# Chunks` section is (re)generated by `/iterator-chunk` and
links every chunk so OKF graph consumers see plan → chunk edges.
...
- **`state.md`** (`type: State`) — the machine's runtime flow state: `mode`
  ...
  (`idle|chunking|testing|implementing|reviewing|escalated|done`),
  `active_chunk`, and `strikes` (a JSON scalar mapping chunk slugs to their
...
- **`usage.md`** (`type: Usage`) — the active plan's token ledger:
  ...
  cache-write/turns plus per-chunk rollups) with a regenerated
  human-readable table as the body. On plan retirement it moves into the
  plan's `chunks/archive/<created>-<slug>/` directory and its grand total is
