#!/usr/bin/env node
/**
 * local-review: plan-features server
...
function start(data) {
  ...
  server.listen(port, '127.0.0.1', () => {
    ...
    exec(`${opener} "${url}"`);
    process.stderr.write(`local-review: plan-features listening on ${url}\n`);
  });

  setTimeout(() => {
    process.stderr.write('local-review: timeout (2h), no adjustments received\n');
...
function buildHtml(data) {
  ...
  return buildChunkPlanHtml(data);
}
...
<head>
  ...
  <title>local-review — plan review — ${data.branch || 'HEAD'}</title>
  <style>
...
function buildChunkPlanHtml(data) {
  const json = JSON.stringify(data);
  return `<!DOCTYPE html>
<html lang="en" data-theme="dark">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>local-review — chunks — ${data.branch || 'HEAD'}</title>
  <style>
    [data-theme="dark"]{--bg:#0d1117;--surface:#161b22;--border:#30363d;--text:#c9d1d9;
...
const D = ${json};
const S = {
  chunks: JSON.parse(JSON.stringify(D.chunks || [])),
...
function renderCards(){
  ...
  S.chunks.forEach(ch => c.appendChild(makeCard(ch)));
}
...
function accept(){
  ...
  if(hasChanges()){
    post({type:'plan-adjustments', branch:D.branch||'HEAD', moves:S.moves, renames:S.renames,
      descUpdates:S.descUpdates, comments:collectComments()}, 'Sent — Claude is updating CHUNKS.md');
  } else {
    post({type:'plan-approved', branch:D.branch||'HEAD'}, 'Chunks accepted — run /lr-implementer to build them');
  }
}
