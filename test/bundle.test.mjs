...
test("backlog index round-trips compact candidate records", () => {
	const raw = backlogIndex([
		{
			id: "dark-mode",
			title: "Dark mode",
			details: "Respect OS preference.",
			kind: "idea",
			selected: true,
		},
	]);
...
test("frontmatter parses block lists and folds commits continuations", () => {
	...
	assert.equal(
		fm.commits[1],
		"sha: a13c9a9deadbeef, kind: fix, date: 2026-07-07",
	);
});
...
test("regenerateAreaIndex rebuilds bullets, preserving heading and prose", () => {
	...
	assert.match(
		out,
		/\* \[A: first\]\(\/pitfalls\/a-first\.md\) - Sorts first\./,
	);
	assert.match(
		out,
		/\* \[Grace timer\]\(\/pitfalls\/grace-timer\.md\) - Cancel-now races\./,
	);
	assert.doesNotMatch(out, /\[Old\]/);
	assert.ok(
		out.indexOf("A: first") < out.indexOf("Grace timer"),
		"sorted by title",
	);
});
...
test("prependLog groups by day, accepts arrays, and honors the header option", () => {
	...
	prependLog(mem, []);
	assert.equal(
		readFileSync(join(mem, "log.md"), "utf8"),
		out,
...
test("validator fails missing frontmatter on root index and concepts", () => {
	...
	const noConcept = validateBundle(
		makeBundle({
			"index.md": '---\nokf_version: "0.1"\n---\n# Memory\n',
			"patterns/index.md": "# Patterns\n",
...
test("commits continuation folding still works for mapping items", () => {
	...
	assert.deepEqual(fm.commits, [
		"sha: abc123, kind: implement, date: 2026-07-06",
	]);
});
