...
created: 2026-07-20
timestamp: "2026-07-20T16:52:09.835Z"
...
The plan is complete. The three accepted feature commits jointly deliver code-exact red-test review and provider-neutral Agent wording across Iterator’s interactive surfaces.

- **Goal coverage:** `review-exact-red-test-source` (`bbb2be9`) adds per-case executable source, complete proposed files, safe rendering, mapping validation, feedback round-tripping, and an approved-artifact write contract before red tests reach disk. `agent-neutral-shell-copy` (`3854fc7`) and `agent-neutral-workflow-copy` (`9544c69`) replace generic Claude actor labels across the shell, server, extension status, and workflow views while preserving explicit Claude Code terminology.
- **Architecture and key decisions:** Shared behavior remains canonical under root `lib/`, shipped copies were synchronized, green-mode behavior remains additively compatible, no external dependencies were added, and approval—not post-approval regeneration—owns the exact red-test source.
- **Scope drift:** One non-blocking incidental cleanup in `3854fc7` changes `test/server.test.mjs` from direct `JSON.parse` to the existing `parseJson` helper. It is unrelated to UI wording but changes no product behavior.
- **Loose ends:** None in the plan commits. All three features are accepted as `done`, no TODO markers were introduced, and the complete suite passed during implementation.
