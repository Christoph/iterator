...
created: 2026-07-20
timestamp: "2026-07-20T18:06:32.753Z"
...
- **Goal coverage:** Complete. `ce7fee4` moves the live price catalog into validated project settings, reuses it in plan-less and later usage gathers, distinguishes an explicit clear from an absent catalog, and keeps active cost calculations current. `8c1ac8d` completes the dedicated Budget save path and bundle bookkeeping.
- **Architecture and key decisions:** Respected. `memory/usage.md` retains plan snapshots for stable archived costs; active gathers prefer project settings; legacy active-ledger prices remain the fallback only before a catalog is saved; no provider prices are fetched or guessed. The general Settings view/schema/writer hide or reject the serialized catalog so Budget remains its editor.
- **Scope and delivery:** Root `lib/` changes are synchronized into shipped skill copies, regression coverage exercises persistence, clearing, snapshot stability, hidden settings, and index registration, and the full suite passed. No unexplained scope drift, external dependency, raw token-accounting change, unfinished feature, or introduced TODO was found.
