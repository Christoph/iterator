...
description: iterator-implement runs a chunk's tests as its goal, only offers Accept-and-commit when green, shows a test badge in the commit-mode UI, and records the implement commit.
status: done
size: medium
lines_estimate: 150
...
- **Step 3 (implement):** after loading the chunk, check `tests` /
  `tests_status`. If tests exist, they are the definition of done: implement,
  run exactly the chunk's test files, and loop implement → run → fix until
...
- **Step 4 (review UI payload):** add per-chunk
  ...
  to the chunk header in commit mode (and in standalone mode when present).
  Badge only — no new interactions.
- **Step 5 (accept-commit):** flip `tests_status: red → green` in the chunk
...
- **Feedback loop (`review-feedback`):** after revising, re-run the chunk's
...
- Step 3 also gains an optional design-quality hook: when the `impeccable`
  skill is available in the harness and the chunk touches UI surface, run
...
* [Schema: tests + commits fields](/chunks/schema-tests-commits.md) — reads `tests`/`tests_status`, writes `tests_status` and `commits`.
