...
- **Flow diagram** (both files): add `/iterator` as the entry point / home
  screen; move `/iterator-test` from "(optional, any time after chunking)" to
  an explicit optional red step between chunk and implement, with implement
...
- **README:** hub section (what the dashboard shows, button enablement), the
  red→green lifecycle in the chunk-sizing/testing area, `tests_status` badge in
  the example chunk, `commits` + trailer lookup note, the new **7777 default
...
- Mention the optional `impeccable` integration (implement runs
  `/impeccable audit`/`polish` on UI chunks when that skill is installed).
...
* [Hub dispatch skill](/chunks/hub-dispatch.md) — documents the finished hub behavior.
* [Dev bind host for Docker](/chunks/expose-bind-host.md) — documents ITERATOR_HOST.
