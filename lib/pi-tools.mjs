...
export function actionToCommand(result) {
  ...
  // Hub (Work tab): step flows, optionally scoped to a chunk. A typed plan
  ...
  if (STEPS.includes(result.action)) {
    ...
    if (result.chunk) parts.push(result.chunk);
  ...
  // okf-init from the hub hero may carry a stashed plan goal: initialize,
 ...
 *   { step, role, chunk, cmd, strike? }  — dispatch cmd as a role turn;
 *                                          strike = chunk whose needs-work
 *                                          counter must be incremented first
 *   { escalate: true, reason, chunk? }   — stop, pause, hand to the human
 *   { done: true }                       — every chunk landed
 *   null                                 — auto mode is not (or no longer) driving
 *
 * Verdict rule: a dispatched agent review either drove accept-commit (chunk
...
export function nextAutoAction(sessionPayload, settings, state) {
  ...
  // notes (they live in the chunk's # Review section).
  if (state.phase === 'reviewing' && state.active_chunk) {
    const ch = (hub.chunks || []).find(c => c.name === state.active_chunk);
    if (ch && ch.status !== 'done') {
      const count = (strikes[state.active_chunk] || 0) + 1;
      if (count >= max) {
        return {
          escalate: true,
          chunk: state.active_chunk,
          reason: `chunk '${state.active_chunk}' failed agent review ${count} time(s) — human intervention needed`,
        };
      }
      return {
        step: 'implement',
        role: 'implementer',
        chunk: state.active_chunk,
        strike: state.active_chunk,
        cmd: `/skill:iterator-implement ${state.active_chunk} --auto`,
      };
    }
  }
  ...
  if (!next) {
    ...
    if (imp?.drafts?.length) {
      return { escalate: true, reason: 'only draft chunks exist — accept the chunk set first (/iterator-chunk)' };
    }
    if (imp?.stuck) {
      return { escalate: true, reason: 'pending chunks remain but none is ready — dependency cycle or missing dependency' };
    }
    return { done: true };
  }
  if ((next.conflicts || []).length) {
    return {
      escalate: true,
      chunk: next.name,
      reason: `chunk '${next.name}' conflicts with recorded decisions (${next.conflicts.map(x => x.decision).join(', ')}) — resolve before implementing`,
    };
  }
  if ((strikes[next.name] || 0) >= max) {
    return {
      escalate: true,
      chunk: next.name,
      reason: `chunk '${next.name}' already failed review ${strikes[next.name]} time(s)`,
    };
  }

  if (settings?.testing_default === 'on' && (next.testsStatus || 'none') === 'none') {
    return { step: 'test', role: 'tester', chunk: next.name, cmd: `/skill:iterator-test ${next.name} --auto` };
  }
  // An implementation diff exists → it is review time; otherwise implement.
  const hubChunk = (hub.chunks || []).find(c => c.name === next.name);
  if (hubChunk?.hasDiff) {
    return { step: 'review', role: 'reviewer', chunk: next.name, cmd: `/skill:iterator-review ${next.name} --agent` };
  }
  return { step: 'implement', role: 'implementer', chunk: next.name, cmd: `/skill:iterator-implement ${next.name} --auto` };
}
...
const ATTRIBUTION_MAP = {
  ...
  okf: 'memory',
  'okf-init': 'memory',
  'okf-consolidate': 'memory',
  'okf-memorize': 'memory',
};

/**
 * Which ledger step (and chunk) a turn belongs to, parsed from the user
 * input that started it (`/iterator-implement auth`, `/skill:okf-memorize`).
 * Returns null when the input is not an iterator/okf command — the previous
...
export function attributionFromInput(text) {
  ...
  const chunk = m[2] && /^[a-z0-9][a-z0-9-]*$/.test(m[2]) ? m[2] : null;
  return { step: ATTRIBUTION_MAP[m[1]], chunk };
}
...
export function usageRowFromMessage(message, attribution) {
  ...
  return {
    ...
    ...(attribution?.chunk ? { chunk: attribution.chunk } : {}),
