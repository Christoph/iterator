...
export function pauseFeatureWave(wave) {
  if (!wave || !Array.isArray(wave.queue) || !Array.isArray(wave.results)) return null;
  return {
...
export function nextFeatureWaveAction(wave, features = []) {
  ...
  while (next.queue.length) {
    ...
    next.results.push({
      feature: featureName,
      status: ['implemented', 'done'].includes(feature.status) ? 'implemented' : 'skipped',
    });
  }
...
export function nextAutoAction(sessionPayload, settings, state) {
  ...
  if (awaiting) {
    return { step: 'review', role: 'reviewer', feature: awaiting.name, cmd: `/skill:iterator-review ${awaiting.name} --agent` };
  }
  ...
  if (!next) {
    if (p.total > 0 && p.done === p.total) {
      ...
      if (!hub.plan.planReviewed) {
        return { step: 'plan-review', role: 'plan_reviewer', cmd: '/skill:iterator-review-plan --auto' };
      }
      return { done: true };
    }
...
const toolSummary = (names) => {
  ...
  const shown = order.slice(0, 6).map(n => (counts.get(n) > 1 ? `${n} ×${counts.get(n)}` : n));
