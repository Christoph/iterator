...
function bundleSubpath(path, env = process.env) {
...
export function isChunkFile(path, env = process.env) {
  const sub = bundleSubpath(path, env);
...
export function isConceptFile(path, env = process.env) {
  const sub = bundleSubpath(path, env);
...
export function isBundleIndexFile(path, env = process.env) {
  const sub = bundleSubpath(path, env);
  if (!sub) return false;
...
export function checkWrite(input, oldContent) {
  if (isBundleIndexFile(input.path)) {
