...
export function tabFor(step) {
  return step === 'knowledge' || step === 'memory-review' ? 'knowledge' : 'work';
}

const TABS = ['work', 'knowledge'];
...
#tabs{display:flex;gap:4px;padding:6px 12px 0;background:#010409;border-bottom:1px solid #30363d}
#tabs button{font:13px -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;color:#8b949e;
  background:transparent;border:1px solid transparent;border-bottom:none;border-radius:6px 6px 0 0;
  padding:7px 18px;cursor:pointer}
#tabs button.sel{color:#c9d1d9;background:#0d1117;border-color:#30363d}
...
export function createSessionServer({ onUnsolicited, log } = {}) {
  ...
  const htmls = { work: null, knowledge: null }; // per-tab documents at GET /view
