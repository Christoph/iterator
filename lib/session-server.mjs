...
import {
  ...
  RUN_ID, STATUS_PATH, TIMEOUT_MS, newRunId, openBrowser, reclaimPort,
  registryPath, takeoverStale,
} from './server.mjs';
...
<div id="overlay">
  ...
  <div id="overlay-note">Knowledge and Usage stay viewable (read-only) in the other tabs.</div>
  <div id="overlay-btns">
...
let seq = 0;
let working = null; // structured working state { text, step?, chunk?, progress?, memories? } | null
...
function applyWorking() {
  ...
  overlayText.textContent = working.text || 'Working…';
  const det = document.getElementById('overlay-detail');
  const detText = [working.step, working.chunk].filter(Boolean).join(' \\u00b7 ');
...
es.addEventListener('working', e => {
  conn.style.display = 'none';
  const d = JSON.parse(e.data);
  working = (d && typeof d === 'object') ? d : { text: String(d || 'Working…') };
  applyWorking();
});
...
export function createSessionServer({ onUnsolicited, onControl, log } = {}) {
  ...
  let working = null;          // { text, step?, chunk?, progress?, memories? } | null while no view is interactive
    ...
    const tryListen = (p, attemptsLeft) => {
      const onError = err => {
        if (err.code === 'EADDRINUSE' && FORCE_PORT && p === startPort
          ...
          // The sandbox publishes only the start port — reclaim it once
          // (a stale/foreign holder makes the dashboard unreachable on 7778).
          reclaimTried = true;
          reclaimPort(p, m => say(m.trim())).then(r => {
            if (r.killed) tryListen(p, attemptsLeft);
            else {
              if (r.reason !== 'session') say(`could not reclaim port ${p} — walking up (the dashboard may be unreachable through the sandbox's port publish)`);
              tryListen(p + 1, attemptsLeft - 1);
            }
          });
        } else if (err.code === 'EADDRINUSE' && attemptsLeft > 0) tryListen(p + 1, attemptsLeft - 1);
  ...
  const api = {
     ...
     * a plain string or a structured { text, step, chunk, progress:{done,
    ...
    showWorking(textOrObj) {
      working = textOrObj && typeof textOrObj === 'object'
        ? { ...textOrObj, text: textOrObj.text || 'Working…' }
        : { text: textOrObj || 'Working…' };
      broadcast('working', working);
    },
