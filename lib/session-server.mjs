...
function setTab(t, v) {
  // A tab switch replaces the iframe, which fires pagehide in an active
  // review. Tell that document this is in-shell navigation before replacing
  // it so its cancel beacon cannot settle the pending review round.
  try { frame.contentWindow.postMessage({ iterator: 'navigate' }, '*'); } catch (e) {}
  tab = t;
