...
.overlap{background:var(--bg);border:1px solid var(--border);border-radius:4px;padding:8px 12px;font-size:13px;color:var(--text-muted);margin-bottom:16px}
.fp{color:var(--text)}
...
function selectFeature(name){
  ...
  detail.innerHTML = memoryPanelHtml() +
    ...
    (D.diffTruncated?'<div class="warn">⚠️ Diff too large for one page — hunks omitted for: '+(D.diffOmittedFiles||[]).map(esc).join(', ')+'. Review those files with git.</div>':'')+
    ((D.uncommittedOverlap||[]).length?'<div class="overlap">ℹ Uncommitted working-tree changes also touch: '+D.uncommittedOverlap.map(esc).join(', ')+' — the review covers only the committed diff.</div>':'')+
    '<div class="meta">'+
...
function fileCard(feat, file, fi){
    ...
    if(file.omitted){
      const or = document.createElement('tr'); or.className='dl';
      or.innerHTML = '<td class="ln"></td><td class="ln"></td><td class="lp"></td><td class="lc">(diff omitted — too large for one page; review this file with git)</td><td class="ci"></td>';
      tbl.appendChild(or);
    }
...
function onPrimary(){
  if(MODE==='commit' && !hasChanges()){
    ...
    post(out, D.source==='commits' ? 'Accepted — Agent is finalizing' : 'Accepted — Agent is committing');
    return;
  }
  post(buildFeedbackObj(), 'Review sent to Agent');
}
...
export function render(data) {
	...
	return renderPage({
		...
		body: BODY,
		clientJs: JS,
		// Commit-sourced reviews (commit-feature already landed the work) only
		// flip the status on accept — the button must not promise a commit.
		primaryIdle:
			commitMode && data.source !== "commits" ? "Accept and commit" : "Accept",
