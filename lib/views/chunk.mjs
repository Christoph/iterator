...
function updateSummary(){
  ...
  document.getElementById('s-over').textContent = cs.filter(c=>estLines(c)>300).length;
...
function makeCard(c){
  ...
  card.innerHTML =
    '<div class="fch"><div class="fchl">'+
      ...
      (c.status==='draft'?'<span class="chip cy">📝 draft</span>':'')+
    ...
    '</div></div>'+
    '<div class="fcb">'+
      (t>300?'<div class="owarn">⚠️ ~'+t+' lines — too big to review (comments/docs don\\'t count). Consider Split.</div>':'')+
      (t>0&&t<50&&!done?'<div class="owarn">⚠️ ~'+t+' lines incl. its tests — too small to be worth its own chunk. Consider Merge.</div>':'')+
...
function splitChunk(name){
  ...
  if(!confirm('Split "'+name+'"? Claude will split it into right-sized (~100-300 line) chunks and reopen this view.')) return;
