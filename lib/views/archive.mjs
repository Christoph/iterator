 ...
 * sections, every feature (notes + review history + commits), and the token
 * totals the plan cost.
 *
 *   input:  { step:"archive", branch, name, title, created, planStatus,
 *             sections: { "<heading>": "<markdown>" },
 *             features: [ { name,title,description,status,size,files,dependsOn,
...
if(D.error){
  ...
  html += '<div class="sec-title">Features ('+((D.features||[]).length)+')</div>';
  (D.features||[]).forEach(c => {
    html += '<div class="card"><h3>'+esc(c.title||c.name)+
      '<span class="chip cmut">'+esc(c.name)+'</span>'+
      (c.status==='done'?'<span class="chip cg">done</span>':'')+
      (c.status==='implemented'?'<span class="chip cy">implemented</span>':'')+
...
function onPrimary(){ post({ type:'action', action:'hub', feature:null }, 'Back to dashboard'); }
`;
