...
/**
 * Copy templates/format.md into the bundle verbatim, once. Both entry points
 * into a bundle seed it — the first plan write and `/iterator-init`'s
 * extensions op — so a knowledge-only bundle still documents its own schema.
 */
function seedFormatDoc(memDir) {
	const dest = join(memDir, "format.md");
...
// EXTENSIONS_BODY lives in bundle.mjs so gather can diff a stored copy
// against it without importing this module (write.mjs already imports gather).
...
function writeExtensions(payload, root) {
	...
	// The contract points at format.md for the metadata schema, so an
	// init-only bundle (no plan yet) must carry it too.
	const seededFormat = seedFormatDoc(b.memDir);
...
function recordPlanReview(payload, root) {
	...
	writeFileSync(join(b.memDir, "plan.md"), joinDoc(fm, bodyText));
	// An agent plan review is the final auto-mode boundary. Persist its terminal
	// state before the agent-end callback so a delayed refresh cannot leave Work
	// claiming it is still executing the completed plan review. Human reviews
	// deliberately leave runtime ownership untouched.
	const autoCompleted =
		payload.by === "agent" && b.state.mode === "auto" && !b.state.paused;
	if (autoCompleted) {
		writeState(
			{
				op: "state",
	...
	return {
		op: "record-plan-review",
		planReviewed: d,
		written: ["plan.md", "log.md", ...(autoCompleted ? ["state.md"] : [])],
		autoCompleted,
		memoryDir: b.memDir,
	};
}
