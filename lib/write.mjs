...
`.replace(/\n{3,}/g, "\n\n");
	...
	if (planBranch) {
		try {
			...
			if (worktreePath) {
				if (existsSync(worktreePath)) {
					...
					gitW(
						branchExists
							...
							: ["worktree", "add", "-b", planBranch, worktreePath],
						b.root,
					);
				...
				gitW(
					branchExists ? ["checkout", planBranch] : ["checkout", "-b", planBranch],
					b.root,
				);
	...
	return {
		...
		...(branchResult?.worktree
			? {
					note: `plan branch ${branchResult.branch} checked out in a separate worktree — continue the work in ${branchResult.worktree} (the current checkout stays on ${curBranch})`,
				}
			: {}),
...
function writeBacklog(payload, root) {
	...
	if (action === "create") {
		...
		changed = { ...input, id, selected: false, created: nowIso(), updated: nowIso() };
		items.push(changed);
	} else if (action === "edit") {
		...
		changed = { ...items[i], ...input, selected: items[i].selected === true, updated: nowIso() };
		items[i] = changed;
	} else if (action === "select") {
		const id = String(payload.id || "").trim();
		if (!/^[a-z0-9][a-z0-9-]*$/.test(id)) fail("backlog id must be a kebab-case slug");
		if (typeof payload.selected !== "boolean") fail("backlog selected must be a boolean");
	...
	prependLog(
		b.memDir,
		payload.log || `**Backlog**: ${action} ${changed.id}${action === "select" ? changed.selected ? " (selected)" : " (deselected)" : ""}.`,
	);
...
function writeDesign(payload, root) {
	...
	const spacingNamed =
		/\b(small|sm)\b[\s\S]*\b(med(ium)?|md)\b[\s\S]*\b(large|lg)\b/i.test(
			s.spacing || "",
		) || ((s.spacing || "").match(/\b[\w-]+\s*[:=]\s*\d+(\.\d+)?(px|rem|em)\b/g) || []).length >= 3;
	if (!spacingNamed)
		warnings.push(
...
function writeSettings(payload, root) {
	...
	prependLog(
		b.memDir,
		payload.log ||
			`**Settings**: Updated ${Object.keys(values).join(", ")}.`,
	);
	return {
		op: "settings",
...
function writeUsage(payload, root) {
	...
	for (const r of rows) {
		if (!r || typeof r.step !== "string" || !r.step.trim())
			fail("usage rows need { step, model?, provider?, input?, output?, cacheRead?, cacheWrite? }");
...
function acceptCommit(payload, root) {
	...
	const changed = gitW(["status", "--porcelain"], b.root)
		...
		.filter((p) => p && !p.startsWith(`${b.memName}/`));
	const globsFor = (slug) =>
		listy(bySlug.get(slug).fm.files).map(globToRegExp);
	...
	for (const e of entries) {
		...
		const paths = [
			...new Set([
				...
				// Uncategorized files the reviewer explicitly assigned here.
	...
	const leftovers = gitW(["status", "--porcelain"], b.root)
		.split("\n")
		.filter(Boolean)
		.map((l) => {
			const p = l.slice(3);
	...
	return {
		...
		committed,
		skipped,
		uncommitted: [...new Set([...skips, ...unassigned])],
		leftovers,
		memorize,
	};
}
...
function commitTests(payload, root) {
	...
	const status =
		payload.testsStatus || (c.fm.status === "done" ? "green" : "red");
...
function writeMemorize(payload, root) {
	...
	let advanceTo = payload.advanceTo || (payload.advance === true ? "HEAD" : null);
...
function writeExtensions(payload, root) {
	...
	prependLog(
		b.memDir,
		payload.log || "**Creation**: Wrote the extension contract (EXTENSIONS.md).",
	);
	return {
		op: "extensions",
...
function retirePlan(payload, root) {
	...
	const usageTotals = existsSync(usageFile)
		? usageGrandTotal(parseUsageTotals(frontmatter(readFileSync(usageFile, "utf8"))))
		: null;
...
function cancelPlan(payload, root) {
	...
	const gitRoot = worktreePath && existsSync(worktreePath) ? worktreePath : b.root;
	...
	// 4. Branch/worktree teardown. Each git failure degrades to a note — the
	//    bundle side of the cancel already landed.
	const removed = {};
	if (worktreePath && existsSync(worktreePath)) {
		if (b.root === worktreePath || b.root.startsWith(`${worktreePath}/`)) {
			notes.push(
				`you are inside the plan worktree — remove it from the main checkout: git worktree remove --force ${worktreePath} && git branch -D ${planBranch}`,
			);
		} else {
			try {
				gitW(["worktree", "remove", "--force", worktreePath], b.root);
				removed.worktree = worktreePath;
			} catch (e) {
	...
	if (planBranch) {
		let cur = gitSoft(["rev-parse", "--abbrev-ref", "HEAD"], b.root);
		if (cur === planBranch && baseBranch) {
			try {
				gitW(["checkout", baseBranch], b.root);
				cur = baseBranch;
			} catch (e) {
				notes.push(
		...
		if (cur !== planBranch && !notes.some((n) => n.includes("inside the plan worktree"))) {
			try {
				gitW(["branch", "-D", planBranch], b.root);
				removed.branch = planBranch;
			} catch (e) {
...
const SCHEMAS = {
	plan: {
		...
		sections: {
			...
			"architecture?": "string (markdown bullet list, built on architecture memories)",
	...
	"record-review": {
		...
		features: [{ name: "slug", status: "approved|changes|question", "note?": "string" }],
		"lineComments?": ["stay with the model — semantic"],
		"by?": "agent|human (default human) — agent reviews are tagged in the history",
