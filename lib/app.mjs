...
import {
  gather, gatherArchive, gatherChunk, gatherKnowledge, gatherPlan,
...
import { render as chunk } from './views/chunk.mjs';
...
const VIEWS = { hub, plan, chunk, test, review, knowledge, 'memory-review': memoryReview, settings, question, usage, archive };

// One-command request form: `{ "gather": true, step, chunk?, project?, extra? }`
...
const GATHERS = {
  ...
  chunk: (o) => gatherChunk(o.project),
  test: (o) => gatherTest(o.project, o.chunk),
  review: (o) => gatherReview(o.project, { chunk: o.chunk }),
  knowledge: (o) => gatherKnowledge(o.project),
  settings: (o) => gatherSettings(o.project),
  usage: (o) => gatherUsage(o.project),
  archive: (o) => gatherArchive(o.project, o.chunk),
  'memory-review': (o) => {
...
const CANCEL_REPORTS = {
  ...
  chunk: 'User cancelled the chunk review. Draft chunks stay on disk as drafts (visible on the hub, not implementable). Write nothing else and stop.',
...
function actionSkill(result) {
  ...
  if (['okf-init', 'okf-consolidate', 'okf-memorize'].includes(result.action)) {
    return result.action;
  }
  const OKF_ACTIONS = ['draft-memory', 'draft-memory-prompt', 'update-memory', 'refresh-format'];
  if (OKF_ACTIONS.includes(result.action)) return 'okf';
  return null;
}
